...
var ScrubTypes = []sqlbase.ColumnType{
	...
	{SemanticType: sqlbase.ColumnType_STRING},
	// TODO(joey): Migrate this to be a string once the JSON column type
	// has been added.
	{SemanticType: sqlbase.ColumnType_STRING},
}
...
func newTableReader(
	...
	// TODO(joey): Add the case for doing a SCRUB check on a secondary
	// index.
	...
	for {
		...
		if tr.isCheck {
			...
			if v, ok := err.(*scrub.Error); ok {
				// FIXME(joey): This is a bit unclear. We overwrite err so that
				// it is only a non-scrubbable error, so the err != nil and
...
func (tr *tableReader) generateScrubErrorRow(
	...
	return sqlbase.EncDatumRow{
		sqlbase.DatumToEncDatum(
			ScrubTypes[0],
			parser.NewDString(scrubErr.Code),
		),
		sqlbase.DatumToEncDatum(
			ScrubTypes[1],
			// TODO(joey): create a reconstructed string of the primary key.
			parser.NewDString("TODO"),
		),
		sqlbase.DatumToEncDatum(
			ScrubTypes[2],
			parser.NewDString(detailsJSON.String()),
		),
	}, nil
}
